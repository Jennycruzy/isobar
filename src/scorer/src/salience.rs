//! In-repo lexical/fact core for the WEATHER_CHECK scorer.
//!
//! The parent crate owns the ABI. This module has no network, filesystem, or
//! randomness; every buffer is fixed-size and every loop is bounded.
...
const STEP_T: f32 = 0.95;
const STEP_B: f32 = 0.04;
...
fn fexp(x: f32) -> f32 {
    ...
    if x > 88.0 {
        return f32::from_bits(0x7f7fffff);
    }
    let t = x * 1.442695041;
...
// Word vectors
// ---------------------------------------------------------------------------
// This compact table is the public salience scorer's bounded semantic tail. It
// supplies topical credit for paraphrases without replacing lexical fact checks.
//
...
fn leading_value(tok: &[u8]) -> f32 {
    if tok.is_empty() || !is_digit(tok[0]) {
        return 0.0;
    }
...
fn raw_blend(q: &[u8], gt: &[u8], ma: &[u8]) -> f32 {
    unsafe {
        ...
        if gt_nums > 0 {
            ...
            while a < tg.n {
                if tg.val[a] > 0.0 {
                    ...
                    let mut b = 0;
                    while b < ta.n {
                        if ta.val[b] > 0.0 && rel_close(tg.val[a], ta.val[b]) {
                            gt_fig_hit += 1;
                            break;
                        }
                        b += 1;
            ...
            while a < ta.n {
                if ta.val[a] > 0.0 {
                    ...
                    let mut b = 0;
                    while b < tg.n {
                        if tg.val[b] > 0.0 && rel_close(ta.val[a], tg.val[b]) {
                            matched = true;
                            break;
                        }
                        b += 1;
                    ...
                    if !matched {
                        let mut b = 0;
                        while b < tq.n {
                            if tq.val[b] > 0.0 && rel_close(ta.val[a], tq.val[b]) {
                                matched = true;
                                break;
                            }
                            b += 1;
...
pub fn raw_score(q: &[u8], gt: &[u8], ma: &[u8]) -> f32 {
    let raw = raw_blend(q, gt, ma);
    if !q.is_empty() && normalized_equal(gt, ma) {
        // Exact answer text still needs a deterministic place in the traffic
        // ordering. Use answer-to-question fit as a small tie-break instead of
        // flattening every perfect reference match to 1.0.
        let query_fit = raw_blend(q, q, ma);
        return clamp01(1.0 - EXACT_TIE * (1.0 - clamp01(query_fit)));
    }
    raw
}

pub fn score(q: &[u8], gt: &[u8], ma: &[u8]) -> f32 {
    let raw = raw_score(q, gt, ma);
    if STEP_T > 0.0 {
        let high = if raw >= STEP_T { 1.0 } else { 0.0 };
        return clamp01((1.0 - STEP_B) * high + STEP_B * raw);
    }
    clamp01(raw)
}
